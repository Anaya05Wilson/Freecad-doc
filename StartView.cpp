...
gsl::owner<MyPushButton*> createNewButton(const NewButton& newButton)
{
    ...
    auto button = gsl::owner<MyPushButton*>(new MyPushButton(newButton.heading));
...
void StartView::configureNewFileButtons(QLayout* layout) const
{
    ...
    auto draft = createNewButton({tr("2D Object"),
                                  tr("Create a draft with the Draft workbench"),
                                  QLatin1String(":/icons/DraftWorkbench.svg")});
    auto createArch = createNewButton({tr("Building"),
                                       tr("Create a building model with the Arch workbench"),
                                       QLatin1String(":/icons/ArchWorkbench.svg")});
    auto createFem = createNewButton({tr("Simulation"),
                                      tr("Create a FEM simulation with the FEM workbench"),
                                      QLatin1String(":/icons/FemWorkbench.svg")});
    auto createMesh = createNewButton({tr("Mesh"),
                                       tr("Create a mesh with the Mesh workbench"),
    ...
    layout->addWidget(createArch);
    layout->addWidget(createFem);
    layout->addWidget(createMesh);

    connect(newEmptyFile, &QPushButton::clicked, this, &StartView::createNewFileClicked);
    connect(openFile, &QPushButton::clicked, this, &StartView::openFileClicked);
    connect(partDesign, &QPushButton::clicked, this, &StartView::partDesignClicked);
    connect(assembly, &QPushButton::clicked, this, &StartView::assemblyClicked);
    connect(draft, &QPushButton::clicked, this, &StartView::draftClicked);
    connect(createArch, &QPushButton::clicked, this, &StartView::createArchClicked);
    connect(createFem, &QPushButton::clicked, this, &StartView::createFemClicked);
    connect(createMesh, &QPushButton::clicked, this, &StartView::createMeshClicked);
}
